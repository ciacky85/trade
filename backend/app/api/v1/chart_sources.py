...
def get_chart_sources(db: Session = Depends(get_db)):
    sources = db.query(models.ChartSource).order_by(models.ChartSource.created_at.desc()).all()
    results = []
    for s in sources:
        results.append({
            "id": s.id,
            "ticker": s.stock.ticker if s.stock else "",
            "company_name": s.stock.company_name if s.stock else "",
            "url": s.url,
...
def create_chart_source(source_in: ChartSourceCreate, db: Session = Depends(get_db)):
    ...
    if not stock:
        stock = models.Stock(ticker=ticker_clean, company_name=f"{ticker_clean} Corp")
        db.add(stock)
        db.commit()
        db.refresh(stock)
