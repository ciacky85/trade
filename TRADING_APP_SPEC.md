...
**Versione Attuale:** v0.7.0

## Changelog
- **v0.7.0**: Risoluzione salvataggio e visualizzazione fonti grafici esterne: aggiunta relazione bidirezionale `Stock.chart_sources` e `ChartSource.stock` in `domain.py` per eliminare l'eccezione `AttributeError: 'ChartSource' object has no attribute 'stock'` su `GET /api/v1/chart-sources/`; implementazione join esplicita (`outerjoin`) nell'endpoint di recupero per garantire compatibilità e performance; auto-riconoscimento piattaforma fonte (TradingView, Finviz, Yahoo Finance, Investing, Web); aggiornamento istantaneo dell'elenco alla creazione/eliminazione e al cambio tab verso "Analisi Avanzata & Fonti Grafici".
- **v0.6.0**: Risoluzione discrepanze grafici e valori non reali nella dashboard: eliminazione del fallback a candele simulate (precedentemente ancorate a 150) in frontend e backend; correzione `NameError: name 'bias_score' is not defined` in `analysis.py` che causava errore 500 sull'endpoint di analisi; integrazione diretta con le API v8 Chart di Yahoo Finance (`query1`/`query2`) per ottenere storico OHLCV autentico al 100% per tutti i timeframe (`1D`, `5D`, `1M`, `3M`, `6M`, `YTD`, `1Y`, `5Y`, `1W`) sia per titoli USA che per titoli di Piazza Affari (`.MI`); supporto valute dinamiche (`€` per Milano, `$` per USA); aggiornamento automatico prezzi live del portafoglio e allineamento simulazione walkforward su dati reali.
